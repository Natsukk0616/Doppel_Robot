...
# Default setting
if len(sys.argv) != 2:
    print("Usage: python ping.py <servo_id>")
    quit()

try:
    SCS_ID = int(sys.argv[1])
except ValueError:
...
# Try to ping the specified SC Servo
scs_model_number, scs_comm_result, scs_error = packetHandler.ping(SCS_ID)
if scs_comm_result != COMM_SUCCESS:
    print("[ID:%03d] ping failed: %s" % (SCS_ID, packetHandler.getTxRxResult(scs_comm_result)))
else:
